...
- **WEEKEND RUSH x2: doubling BUILT B57c** (Game `RewardCalculator`, Victory-only, doubles gold + account/tower/battlepass XP; **drops + challenge fragments NOT doubled** — a scope choice). Config is a Game-LOCAL byte-identical copy (`f2400aca` both Places). **PENDING: promote `WeekendRushConfig` to shared canon (both Places + manifest) + drop the duplicate, and `git commit` — all deferred, shell down.** Confirm timezone (UTC) + whether to also double drops/fragments. `docs/proposals/2026-09-10-weekend-rush-game-doubling.md`. **LUCK-ON-REROLLS: BUILT B57b** — best-of-N, Lobby-local (`RerollLuckConfig.RollCount`: best-of-2 at 25/50/75%, best-of-3 at 100%; N=1 unchanged). Trait keeps the rarest, stat the highest-total. Crossed AD-Traits with user go-ahead. Tunable in one function; weight-bias (summon-consistent, shared-canon) alternative still in the proposal. `docs/proposals/2026-09-10-luck-on-rerolls.md`.
