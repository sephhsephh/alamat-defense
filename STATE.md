...
- **Shared canon** (`shared/manifest.json`): **39 entries = 32 modules + 7 templates**; B45 re-hashed `ItemCatalog`->**`2ee5f976`** (B50 +15 crafting items) + `RewardScalingConfig`->**`e0a3bc2d`**; B51 +`MetaConfig` (shared, `5166d377`) and `MetaMath` now in the GAME; B52 +`ChallengeConfig`/`MatchModifiersConfig` (shared, both Places). Checked start AND end by
