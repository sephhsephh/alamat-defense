...
| Unit capacity (B72) | AD-Game (crossing AD-Lobby, user go-ahead) | Lobby | Studio (Lobby) `RS.Configs.Meta.UnitCapacityConfig` + `SSS.Server.Meta.UnitCapacityService` (one writer of `Data.UnitSlotsPurchased`, schema v8) + the check in `SummonService`; `docs/systems/unit-capacity.md`. UI surfaces (Units screen + Shop row) = B73. |
