...
- **Shared canon: 42/42 PRESENT here** (B58; `MetaMath` reached the GAME at B51, so it is no longer a Game gap). Hashes in `shared/manifest.json`. **B76: `ProfileTemplate` `d3d4e63c` -> `461fed3e` (schema v9, `HudCurrencies`), mirrored byte-identical to the Game and disk in the same session; all 35 manifest modules re-hashed equal in BOTH Places.** The HUD currency bar is now player-configurable (gear -> `CurrencyConfigGUI`, 3 slots, `HudCurrencyConfig`/`HudCurrencyService`) and a pinned pill can be an ITEM -- which is why `GrantService`'s Item grant and `SpendItems` now fire `CurrencyChanged` too (`docs/systems/hud-currencies.md`). **B77: `TierConfig` `eee2b3ad` -> `4aa53b25` (the user's NINE tiers + their authored gradient stops/rotation; Exclusive/Bathala retired) and `UIKitHotbar` `b2287846` -> `55c3df63` (a V3 path for the user's authored hotbar). THIS PLACE RAN THE V2 HOTBAR UNTIL THE USER PASTED THEIRS IN LATER THE SAME SESSION** — it now runs the same V3 path as the Game; the V2 kit-clone path is dead code in both Places, removal pending its own pass. **B79: `UIKitUnitCard` `58f014de` -> `51f55122`, `UIKitHotbar` `998b2bca` -> `c0091d72`** (both Places + disk): viewport idles actually MOVE (`Animator:StepAnimations` on RenderStepped; a default idle for rigs with none) and the tier stroke spins/scrolls/breathes at a per-slot random rate. `docs/systems/hotbar.md`. Every tier-coloured screen here (Units, Index, Summon, Ascension, rerolls) now draws the new palette.
...
- **AD-UI:** ⚠ **B79: `RS.UnitModels` NOW HOLDS THE REAL RIGS** -- the user copied all 8 display models over from the Game, so cards/hotbar/previews here draw the actual unit, not `Placeholder`. **A NEW UNIT MUST BE COPIED INTO BOTH PLACES** (user rule; Game `TowerModels` + this `UnitModels`, same name and `IdleAnim` attribute) or it previews as `Placeholder` here again -- this Place owns no `TowerConfig` to fall back on. Viewport rigs play their idle through `UIKit.UnitCard.playIdle`, which **STEPS the Animator every frame** (a ViewportFrame renders but does not simulate) and falls back to `UnitCard.DefaultIdleAnim` when a rig has no `IdleAnim`. `ItemHoverCard` split. `QuickSellButton` wired B31; `FavoriteButton` + `ItemHoverCard` split. `QuickSellButton` wired B31; `FavoriteButton` +
