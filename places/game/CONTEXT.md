...
- **B77: THE HOTBAR IS THE USER'S AUTHORED `StarterGui.Hotbar`** (their `NEW Hotbar`, renamed; the old kit hotbar and `Hotbar - old` are DELETED). `HotbarFrame.Slots.HotbarSlot1..6` + `UnitHoverPreviewTemplate` are painted in place by the shared `UIKit.Hotbar`'s new **V3 path** (`b2287846` -> `55c3df63`), and the tier colours come from `TierConfig` (`eee2b3ad` -> `4aa53b25`), which now carries the user's **nine** tiers with the `Stops`/`Rotation` they authored. Hover tweens `InnerStroke`'s gradient to white. Afford/limit dimming goes through `handle.setOverlay`, never `Main`/`BG` (those nodes are gone). ⚠ a LOCKED slot can still hold an entry (the Lobby's auto-loadout fallback over-fills) — the controller must not clear its dim. `docs/systems/hotbar.md`. **B79 (`UIKitHotbar` `998b2bca` -> `c0091d72`, `UIKitUnitCard` `58f014de` -> `51f55122`): THE SLOT VIEWPORTS ANIMATE** -- `UnitCard.playIdle` steps the rig's Animator every RenderStepped (a ViewportFrame renders but does not SIMULATE, which is why B78's track played and nothing moved) and falls back to `UnitCard.DefaultIdleAnim` for a rig with no `IdleAnim` attribute. The tier stroke moves too: per-slot random spin, multi-stop tiers scroll their gradient offset, flat tiers breathe toward white, ONE RenderStepped connection for all six slots, and a HOVERED slot stops writing colour so it never fights the hover tween. ⚠ **a new unit's rig belongs in `RS.TowerModels` HERE *and* `RS.UnitModels` in the LOBBY** (user rule) -- same name, same `IdleAnim`. **B80: BOSS BAR = the user's IMAGE layout** (`StarterGui.BossHealth`: BarBG frame / FillBG / Fill / DamageTrail). `HealthBarFX` never resizes an image fill -- it crops it with a `FillMask` UIGradient across the channel (Fill attrs `ChannelLeft`/`ChannelRight`, else the authored DamageTrail box) -- and only scales a Frame layer's WIDTH. The `BarTemplate` inside `Container` is the designer's PREVIEW and is hidden at boot; the live template is `BossHealth.BarTemplate`. ⚠ z-order matters: FillBG 0 < DamageTrail 1 < Fill 2, or the damage flash hides behind FillBG.
