...
**TIERS ARE NOW THE USER'S NINE**, low to high: Common, Uncommon, Rare, Epic, Legendary, Mythic, Limited, Secret, **Alamat**. Each carries the `Stops` (keypoint TIMES included) and `Rotation` of the gradient they authored on `HotbarSlot#.InnerStroke`, because copying only the colours would have flattened a 45-degree three-stop sheen into an even fade. `Colors` is derived from `Stops`, so every existing caller is unchanged. **`Exclusive`/`Bathala` RETIRED** (no weight, no unit, no reference outside comments). **B27's derived-darker-second-colour rule is RETIRED**: four tiers are authored FLAT on purpose and auto-darkening would overwrite a hand-tuned palette. SellValue gained Uncommon 15 / Limited 700 / Alamat 2500.

**THE HOTBAR IS THE USER'S AUTHORED GUI.** `NEW Hotbar` became `Hotbar`; the old kit hotbar and the legacy `Hotbar - old` are DELETED (user's call). The design lives in each Place's ScreenGui and the shared `UIKit.Hotbar` grew a **V3 path** (recognised by `HotbarSlot1`) that paints those authored slots in place: tier gradient, viewport model, name, TIER label, trait icon, "Lv. N", the REAL ₱ cost, and an `Overlay` for locked/empty. The V2 kit-clone path is untouched and still drives the LOBBY until its copy of the GUI arrives. **Hover = the InnerStroke gradient tweens to WHITE and back** (a ColorSequence cannot be tweened, so a NumberValue is tweened and every keypoint lerps). The new `UnitHoverPreviewTemplate` shows name (+ascension), tier, level, DMG/SPA/RNG base values with this unit's roll GRADES, and Trait/Element/Shiny chips -- Element reads "None" until the element system lands.

**TWO REAL BUGS CAUGHT LIVE:** the Game controller cleared the dim on any slot holding an entry, which wiped the LOCK dim off slots the Lobby's auto-loadout fallback over-filled (B43's note); and the old dimming poked `Main`/`BG`, nodes the new slot does not have -- both replaced by `handle.setOverlay`, which also closes the user's own TODO ("the background transparency becomes 0 on start"). The nine authored slots were a PALETTE: 7-9 deleted, the row draws `LoadoutConfig.MaxSlots` (6).

**B77 FOLLOW-UP (user's playtest; `UIKitHotbar` `55c3df63` -> `4df4b120`):** (a) **clicking a slot did nothing while keys 1-6 worked** -- the authored buttons ship `Active = false`, and a GuiButton that is not Active never fires `Activated` (the keys come from UserInputService and never touch the button). V3 now forces `Active = true` and connects `MouseButton1Click` as well, debounced 0.05s. (b) the preview's tier colour moved to **`UnitFrame.UIStroke` only** -- the card's outer strokes are the design's frame and stay as authored. (c) **viewport cameras are left alone** (user): `showModelKeepCamera` never creates, moves or assigns a Camera; it records the authored placeholder's pivot as an attribute on the ViewportFrame and pivots each swapped-in model to that spot. (d) **the authored VPCamera never reached the player**: a `Camera` inside `StarterGui` does NOT replicate into `PlayerGui`, so `CurrentCamera` was nil on every slot and Roblox's default framing rendered. The framing now travels as the replicating attributes `HotbarCamCFrame`/`HotbarCamFOV` on the Slots frame (baked from the user's camera; **re-bake after moving it**), and `ensureCamera` builds a camera from them for any viewport without a live one. **The spot the camera looks at is baked with it** (`HotbarModelPivot`): with a shared camera but each slot standing its model on its own old placeholder's pivot, five of six models sat ~59 studs out of frame. Verified after the user re-aimed the camera: all six slots and the hover preview frame the unit identically.

**LOBBY MIGRATED (same session).** The user pasted the ScreenGui in as `HotbarV2`; it is now `Hotbar`, the old kit hotbar is deleted, and the Lobby runs the same V3 path. ⚠ **THE PASTE CARRIED THE GAME'S CONTROLLER** -- a LocalScript is a child of the ScreenGui, so it travels with a copy; left alone it would have required the Game's `PlacementController` and waited forever on `Remotes.Match.LoadoutAssigned`. Replaced with the Lobby's own (click opens the Units screen on that unit; models from `RS.UnitModels`). The baked camera attributes travelled with the paste as intended. Proven live: Farm/RARE ₱150, Babaylan/EPIC ₱300, Meteor/LEGENDARY ₱300 with the right gradients and rotations, locked slots showing their unlock level, hover preview reading Babaylan EPIC Lv.100 DMG 20 (D) / SPA 2.5 (A) / RNG 22 (B) with its Godly trait chip, and a click opening the Units screen. NOTE: `RS.UnitModels` holds only `Placeholder` in the Lobby (pre-existing), so every Lobby viewport draws the placeholder rig until real display models land there.

Proven live: four filled slots with the exact authored gradients, locked slots dim at 0.25 showing their unlock level, hover whitens, preview reads Necromancer MYTHIC Lv.20 DMG 28 (D) / SPA 1.1 (B) / RNG 22 (D), click starts placement. **USER REPUBLISHES BOTH PLACES** (shared canon changed in both).
...
**THE FLOW, AS BUILT.** An empty SLOT with a "+" opens a single-select unit PICKER (search, tier-sorted, each card badged with the unit's CURRENT active trait -- what you are about to gamble). Choosing fills the slot and closes the picker. Around it: **Show Chances** toggles a TRAIT INDEX with real odds and this unit's pity bars; **Filters** opens the trait grid where green = a trait you are HUNTING; **Reroll** rolls once, confirming first if the unit's active trait is one you are hunting; **Instant Roll** rolls until it lands a hunted trait or spends the budget; and the bottom bar swaps ACTIVE <-> STORED for free.

**FOUR NEW SERVER PATHS, ONE SHARED ROLL.** `rollOnce` was extracted so `RerollTrait` and `InstantRollTrait` cannot drift -- a bug fixed in one would otherwise survive in the other, and Instant Roll is exactly the path nobody watches closely. It rolls and writes but never SPENDS, because Instant Roll must pay per iteration and stop the moment it cannot.

⚠ **INSTANT ROLL REFUSES OUTRIGHT WHEN NO FILTER IS SET, and that guard is the feature.** With nothing to stop on, the loop's only bound is the budget -- so one careless press would spend the player's entire stock and leave them wherever the last roll fell. A feature whose worst case is "silently burned everything you owned" must not be reachable by accident. There is also a hard server ceiling of 1000 per call: this is ONE RemoteFunction invocation and the client is blocked until it returns, so the cap bounds the CALL, not the cost.

`SetTraitFilters` validates against `TraitDefinitions`, writes SPARSE (only `true`), and refuses `"None"` -- hunting None would make Instant Roll stop on its first roll, every time. Filters ride along with `GetUnitViews` rather than opening a second profile read path (ADR-0004).

**PROVEN LIVE.** Empty state, then Archer selected: `TRAIT "Godly"`, stats `+300% DMG, +50% RNG, +100% CRIT, 1 Placement Limit`, `ACTIVE=Godly STORED=NONE`. Trait Index: Blitz 6.73%, Sniper 6.73%, Deadeye 2.10%, Godly 0.253% -- computed from the weights, so the screen cannot advertise odds the roll does not use -- with pity bars on Deadeye and Godly only.

**PER-UNIT PITY, DEMONSTRATED BY SWITCHING UNITS:** Archer `Deadeye 0/75, Godly 0/600`; Necromancer `5/75, 5/600` after its five rerolls; back to Archer `0/75, 0/600`. Same trait, different progress per tower -- which is the entire point of putting the counter on the UnitInstance.

Server contract, exercised against the real remotes: Instant Roll with no filters -> `refused: no_filters`; `SetTraitFilters({Godly, Deadeye, None, Nonsense})` kept exactly `{Deadeye, Godly}`; swap round-tripped `Godly/nil -> nil/Godly -> Godly/nil`; Instant Roll budget 40 spent **exactly 40**, advanced pity 0 -> 40 on both capped traits (one tick per roll, never per best-of-N candidate) and moved the balance 55 -> 15. Filters and the burned trait both survived a Play restart. The confirm popup fired verbatim -- *"Are you sure you want to reroll this trait **Sniper** on unit **Archer**?"* -- and "Keep it" left the trait AND the balance untouched.

**⚠ THREE BUGS THE ACCEPTANCE RUN CAUGHT, ALL MINE, ALL IN THE AUTHORING:**

1. **A dialog Card is a plain Frame, and a plain Frame does not consume input.** The Scrim behind it is a full-screen TextButton, so any click landing on a card's own padding fell THROUGH and dismissed the dialog -- which is what produced a stray `TraitFilters -> 0 hunted` early in the run. `Active = true` is what makes a Frame swallow clicks; a modal has to.
2. **`UIListLayout.SortOrder` left at its default sorts by NAME**, so the action row came out alphabetical (Filters, InstantRoll, Reroll, ShowChances) instead of the LayoutOrder set on it. Now `Show Chances | Reroll | Instant Roll | Filters`, matching the reference. Fixed on the Instant Roll presets row too, where Min/Half/Max happens to sort correctly by name today -- which is exactly why it would have bitten silently later.
3. **CLICK-OUTSIDE-TO-CLOSE IS DELETED FROM THIS SCREEN, and the reason generalises.** The other Lobby screens compare `input.Position` against `panel.AbsolutePosition`. Those are in DIFFERENT COORDINATE SPACES -- input includes the topbar GUI inset, AbsolutePosition does not. Measured live: the pointer over a button reported y=578 while the button's rect said y=520, a 58px disagreement equal to `GuiService:GetGuiInset().Y`. It survives elsewhere only because those panels are large and centred so the shift rarely flips the test; here it was reached and closed the whole screen mid-interaction. Removed rather than patched: there is a CloseButton and every overlay dismisses on its own scrim, and **a dismissal rule that can be silently wrong is worse than one fewer way to close a window.** ⚠ **The same latent bug is in `AscensionController`, `StatRerollController` and `IndexController` -- NOT fixed here (different owners, pre-existing). Recorded for whoever owns them.**

**167 authored Instances**, built in the Edit datamodel and tagged `B70Built` so the builder is idempotent -- a re-run destroys the tagged nodes rather than stacking duplicates. **The pre-B70 `Grid` / `Detail` / `EmptyLabel` / `Subtitle` are HIDDEN, not destroyed**: they are the designer's authored art, and deleting someone's instances to make room is the kind of unasked-for change this project forbids. Delete them by hand once happy.

**STILL TO COME (B71):** the Robux reroll bundles against stubbed product ids. The trait ICONS in the Index are `ImageLabel`s with no asset yet -- deliberately blank rather than showing placeholder art, and they fill themselves the day `TraitDefinitions` grows an `Icon` field. No schema change (still v7). **USER: republish BOTH Places -- B69's schema bump still needs it.**

## 2026-09-16 [both] B69 -- AD-Game (crossing AD-Traits/AD-Lobby with the user's go-ahead): **SCHEMA v7 -- one trait becomes two slots, and pity becomes per-unit.**

Foundation for the trait-reroll screen rebuild the user asked for. **No UI shipped this session on purpose**: the screen needs data that can hold a bench trait and a per-unit pity counter, and building the controller against v6 only to rewrite it against v7 would have been the expensive way round. B70 is the screen; B71 the Robux bundles.

**THE USER'S SPEC, SETTLED IN SEVEN QUESTIONS** (all recorded so the next session does not re-litigate): I author the new UI Instances in the Edit datamodel (real authored instances, never script-generated); **in scope** = per-unit pity + active/stored slots + Instant Roll + saved filters; the reroll keeps costing the `TraitRerollToken` ITEM with Robux bundles wired later against stubbed product ids; the unit picker is **SINGLE-select**; **a filtered trait is one the player WANTS** (Instant Roll stops on it, and rerolling a unit whose ACTIVE trait is filtered asks first); pity is a **hard guarantee at cap, then reset**.

**WHAT CHANGED IN THE SAVE SHAPE.** `UnitInstance.Trait` SPLITS into `ActiveTrait` + `StoredTrait`. Only the active one is ever read -- by the stat resolver, by `TraitRegistry.GetEffectivePlacementLimit`, and by every display; the bench changes only on an explicit swap. Plus per-unit `TraitPity` and the top-level `TraitFilters`.

⚠ **`Migrations[6]` IS THE FIRST STEP SINCE v1 -> v2 WITH REAL WORK, AND THE REASON IS THE TRAP TO REMEMBER.** Every step from v2 to v6 was a deliberate no-op because `Reconcile()` fills missing TOP-LEVEL keys before `Migrate()` runs. **It does NOT descend into `Units[uuid]`.** So `TraitFilters` really was free, while the three new UnitInstance fields had to be written BY HAND, per unit. Anyone adding a field inside `Units` later should not assume the v2..v6 pattern.

Two judgement calls inside the migration, both the honest direction: `Trait` is **CLEARED** after the copy, so there is exactly one answer to "what trait does this unit have" -- leaving it would create a second source of truth that goes stale on the first reroll, which is the class of lie section 8 exists to prevent. And **pity starts at ZERO for everyone** rather than back-dating a roll history nobody actually spent.

**THE WIRE FIELD DELIBERATELY DID NOT CHANGE.** A unit view still carries `Trait`, and it now always means the ACTIVE one; `StoredTrait` and `TraitPity` join it as NEW view fields for the screen. That kept the blast radius inside the profile layer instead of rippling through every client display and the whole Game placement chain -- and it is not a fudge, because the wire shape already differs from the save shape (views carry `Name`, `Tier`, `Grades`, none of which are stored).

**THE LAYERING SAVED MOST OF THE WORK, AND IT IS WORTH KNOWING WHY.** A `.Trait` sweep across both Places found 23 code sites, but only SIX build a real `UnitInstance`: `GrantService.newUnitInstance`, `StarterChoiceService.newUnitInstance`, `PlayerInventoryService.GetUnit`/`GrantUnit`/the dev-seed path, and the v1 migration. `SummonEngine` and `SummonService` needed **no change at all** -- their `Trait` is a GRANT-OPTS key, a call argument rather than a saved shape, and opts keeps its caller-facing name. Changed: `GrantService` (storage + view), `LobbyServices` (view), `StarterChoiceService`, `TraitRerollService`, and in the Game `PlayerInventoryService` (4 sites) + `LoadoutValidator`. A post-sweep in both Places confirms the only surviving `.Trait` reads are INSIDE the migration steps, which is exactly where the old shape must still be named.

**PITY, AND WHY IT LIVES WHERE IT DOES.** The caps sit in `TraitDefinitions` beside `Weight` -- that file is already the single definition of trait rarity, and a cap stored anywhere else could silently drift from the weight it exists to compensate for. The maths is PURE in `TraitRegistry` (`PitiedTraits`, `PityCap`, `PendingGuarantee`, `Rarer`, `AdvancePity`); the SERVICE owns every write, the same split this project uses everywhere. Two load-bearing details:

- the pitied list is sorted **rarest first WITH A NAME TIEBREAK**, because `pairs()` order is not deterministic in Luau -- without the tiebreak two servers running identical code could disagree about which guarantee wins when two caps fill together;
- **a guarantee is a FLOOR, not a ceiling.** `TraitRerollService` takes the RARER of the guaranteed trait and the Luck buff's best-of-N (B57), so a lucky Godly still beats a guaranteed Deadeye and neither feature can make the other worse. `AdvancePity` ticks ONCE PER REROLL -- one token spent, one tick -- never once per best-of-N candidate.

⚠ **THE TWO PITY CAPS ARE PLACEHOLDERS AND ARE THE USER'S CALL** (`Deadeye = 75`, `Godly = 600`). They follow ~1.5x the expected rolls at current weights (Deadeye 25/1188 = 2.104% -> ~48 expected; Godly 3/1188 = 0.2525% -> ~396). Retune either by editing ONE number; a longer cap makes a trait rarer in PRACTICE without touching `Weight`, which would also move the odds the Trait Index displays. Blitz and Sniper get no cap on purpose -- at 6.7% each a pity bar would be noise. **The reference screenshots the user supplied are from a different game with eleven traits; this game has five, and B69 built against ITS trait table, not that one.**

**PROVEN LIVE** -- the dev profile was at v6, so joining the Lobby migrated it for real:
`[DATA] v6->v7: 3 unit(s) Trait -> ActiveTrait, 8 seeded TraitPity` -> `Migrated SuperiorBeing_S's profile forward 1 step(s) to v7` -> `Profile v7 loaded` -> `Lobby boot: save-schema v7`. `TraitRerollService ready (... Schema v7 pity, PER UNIT: Godly@600, Deadeye@75)`. `ScreenBootWatchdog: 40/40`, no errors.

**SHARED CANON, three modules re-hashed and mirrored in the SAME session**: `ProfileTemplate` **`83633e44` -> `8f6520b4`**, `TraitDefinitions` **`56e81e37` -> `b6b6b120`**, `TraitRegistry` **`eca681ad` -> `ad859588`**. Disk canon was proven IN SYNC before the edits and byte-identical to both Places after. Baseline asserted so B67/B68 could not be reverted. Manifest stays at **42 entries**.

⚠ **DOC CORRECTION IN THE SAME PASS:** `STATE.md` and `places/lobby/CONTEXT.md` still claimed **schema v5** (B48's `91ffab78`) -- they were never updated when B55 shipped v6, so they had been a version behind for fourteen sessions. Brought to v7 here rather than layering a new number on a stale one.

**STILL TO COME (B70):** the filter gate, Instant Roll and the active/stored swap all exist in the schema but have NO service path or UI yet -- `TraitRerollService` still does a plain single reroll. **USER: REPUBLISH BOTH PLACES -- a schema bump is a both-Places publish.**

## 2026-09-16 [both] B68 -- AD-Game (crossing AD-UI/AD-Lobby with the user's go-ahead): **the placeholder price is gone from ALL SIX remaining screens, and the switch that allowed it is deleted.**

User: *"also fix the prices in the unitsgui."* The Units screen was not alone -- **six** Lobby surfaces were still rendering `PlacementPrice` from the TEMPLATE'S AUTHORED number, and the user chose to sweep all of them and retire the flag rather than fix the one they named.

**WHAT WAS STILL LYING.** B67 gave the hotbar a real price in both Places, but `Motion.SHOW_PLACEHOLDER_PRICES` -- the B27c testing override -- was still feeding the artist's figure to `UnitsGUI`, `IndexScreen`, `SummonScreen`'s selection chips, `AscensionController`, `TraitRerollController` and `StatRerollController`. Same number on every card, in every one of them, wrong for all of them. The GAME was already clean: after B67 the hotbar was its only consumer, so the flag was dead there and alive only in the Lobby -- which is exactly the shape a half-finished fix leaves behind.

**THE PRICE IS NOW THE CARD KIT'S JOB.** `UIKit.UnitCard` gains `formatCost` (peso + thousands separators) and `paintPrice(root, towerId)`: **one lookup, one format, one hide rule** for all SEVEN surfaces that draw this field across both Places. B67 had put the lookup and the formatting inside `UIKitHotbar`, which was right when there was one consumer; the moment six more needed the identical pair it belonged in the module that exists for precisely this reason (B34 merged four copies of the camera-framing formula into `UnitCard` for the same argument). **`UIKitHotbar` was refactored onto it in the same session** -- so the Game's hotbar slot and the Lobby's Units card are not two implementations that agree today, they are the same function.

**HIDE-NEVER-INVENT, ENFORCED BY SHAPE.** `paintPrice(root, nil)` HIDES the label. That is how an empty or locked hotbar slot, a Summon chip that is an item rather than a tower, and any id missing from the catalog all ask for nothing instead of for a `0` -- a zero would claim the tower is free. There is no longer any code path that can render a price the data does not support.

**`Motion.SHOW_PLACEHOLDER_PRICES` IS DELETED, NOT SET FALSE.** Its own comment carried the instruction: *"when real prices land, set this false, delete this block, and fill the label from the real source."* B67 landed the source; B68 removed the switch along with all seven readers. Deleting beats disabling here -- **a dormant flag is an invitation to render a fabricated number again**, and there is now nothing to flip. Three controllers (`Ascension`, `TraitReroll`, `StatReroll`) required `Motion` for **that field alone**, so their requires went with it; they never used Motion for motion.

**VERIFIED LIVE, FIVE OF THE SIX SCREENS, plus the Game re-proved after the refactor:**

- **Game hotbar** (unchanged behaviour, re-run after the refactor): Archer **100**, Necromancer **400**, Warchief **350**, Farm **150**; slots 5 and 6 read `locked=true` with **no price at all** -- the nil path.
- **Lobby Units** (opened by clicking the real HUD button): 16 cards, e.g. Necromancer **400**, Meteor **300**, Warchief **350**, Babaylan **300**, Farm **150**, Mage **200**.
- **Index**: all 8 catalogued towers -- Archer 100, Knight 150, Mage 200, Farm 150, Babaylan 300, Meteor 300, Warchief 350, Necromancer 400. **This is the whole `Costs` table on screen**, and the Game's boot validator proves that table against the live `TowerConfig.Cost`, so it is a full cross-Place reconciliation in one reading.
- **Ascension** (1 eligible unit): Necromancer **400**. **TraitReroll** and **StatReroll** (8 units each): Necromancer **400**, Meteor **300**, Warchief **350**.
- Cross-Place overlap: **Necromancer 400, Warchief 350, Farm 150 read identically in the Game and the Lobby.**
- No errors in either Place; the Lobby's `ScreenBootWatchdog` reported **40/40 boot scripts finished**, so none of the six edited controllers hung.

⚠ **THE SIXTH SURFACE WAS NOT EXERCISED LIVE.** `SummonScreen`'s selection chips are built in `openChoice`, and on this profile the banner's **"Change featured unit" button did not open the choice overlay** -- the overlay stayed `Visible = false` with the template's authored title still in place, and no refusal was printed. That button is `SummonController`'s own behaviour and **B68 did not touch it**; it is recorded here as an observation for the user, NOT diagnosed and NOT "fixed". The chip's price call is the same single `UnitCard.paintPrice(chip, id)` proved on the other five.

**SHARED CANON: THREE modules re-hashed and mirrored in the SAME session** (no stale `deployed.<Place>`): `UIKitUnitCard` **`bd2421c5` -> `bedb105e`** (9,979 bytes), `UIKitMotion` **`2d217ede` -> `ed85d82c`** (24,497 bytes), `UIKitHotbar` **`5b9f9260` -> `b2287846`** (20,259 bytes). The full chain was proven by hash before anything was written: disk canon **pre**-edit matched the manifest AND both Places; the same edits were applied to disk; disk **post**-edit matches both Places byte-for-byte. Baseline asserted before touching the manifest so B65/B66/B67 could not be reverted. Manifest stays at **42 entries**.

**A TOOLING NOTE WORTH KEEPING: the drift check can run entirely in raw Luau.** B64 found Studio's script-capability sandbox refuses every `require` from the MCP thread, and B68 hit a second wall -- it also refuses to **fire a BindableEvent** (`ClientEvents.OpenIndex` was rejected on Capabilities). But `.Source` reads and `bit32` need neither, so the FNV-1a check now runs *in-Place* in one call, with the 16-bit-half `mul32` B64 worked out (a naive `h * 16777619` overflows a double past 2^53 and returns garbage). It was validated against a CONTROL -- an untouched module whose hash the disk pipeline already knew (`UnitStatsCatalog` 4484 / `ff870013`) -- before any new hash was trusted. Screens were opened by **clicking their real buttons** via `user_mouse_input` with `instance_path`, which needs no capability at all.
...
**`WeekendRushConfig` PROMOTED TO SHARED CANON.** `shared/src/WeekendRushConfig.luau`, manifest **41 -> 42**, `deployPath ReplicatedStorage.Configs.Meta.WeekendRushConfig`, owner AD-Meta. Header rewritten from "Lobby-local FOR NOW" to shared canon and re-hashed AFTER that edit: `f2400aca -> 44c549f0`, byte-identical in both Places and on disk. Drift **42/42 in each Place**, compared field-by-field against BOTH `hash` and `deployed.<Place>`. `HashShared`'s MODULES table updated in both Places, TOOLVERSION `B41-1 -> B58-1`.

**TWO USER DECISIONS APPLIED.** (1) **Drops + challenge fragments NOW DOUBLE** -- B57c's open scope question closed YES. One scalar over each drop's `Count`, placed where the stage roll, the Insane items and the day's fragments are ALL already in `drops`, so there is no second place to keep in step; ids and `ItemCatalog` Kind routing untouched, so a doubled CURRENCY still lands in `Data.Currencies` (B45). (2) **The window moved UTC -> UTC+8 (Asia/Manila)**; edges verified open Fri 00:00 Manila, shut one second before, open through Sun 23:59, shut Mon 00:00.

**BOTH B57 FOLLOW-UPS VERIFIED LIVE, which is what this session was for.** Weekend Rush on a REAL 15/15 Victory: `gold 628` against a 100-300 band (a 314 roll doubled) | `BP XP +250` | `WeekendRush x2`. Drops, deterministically, on an Insane Victory: `[DATA] Drop: StatRerolls x2 -> Currencies.StatRerolls = 2` where Insane grants x1. Luck-on-rerolls: `DevLuck=100` through the real `RerollTrait`/`RerollStats` remotes on a real owned unit -> `bestOf=3` in three `[DATA] TraitReroll` and three `[DATA] StatReroll` lines. Magnitude KEPT as built (user).

**DRIFT FINDING 1 -- the Lobby was 40/41 at bootstrap.** `MovementConfig` read `19421017` against a manifest of `9c7cbd32`: the USER had re-tuned `SprintSpeed` 26 -> 56 and `DashSpeed` 70 -> 300 in the Lobby Studio. Asked before touching it (the 2026-08-16 rule); they confirmed and said keep. RECORDED as canon on the B22 `ItemCatalog` precedent -- `shared/src` rebuilt from disk canon with exactly those two edits and PROVED byte-identical to the live Lobby copy by hash BEFORE writing, then the Game brought to the same bytes in the same session, so unlike B22 no stale `deployed.<Place>` is left behind.

**DRIFT FINDING 2 -- the canon drift tool was itself stale, and this one is the dangerous one.** `tools/hash_shared.luau` (CANON per CLAUDE.md) was missing `MetaConfig` (B51), `MatchModifiersConfig`+`ChallengeConfig` (B52) and `MovementConfig`+`MovementController` (B54): all five were added to the IN-STUDIO `HashShared` at the time and never back-ported. The repo tool therefore checked **36 entries while the Places held 41**, and the module header's promise that "the repo tool always works as a fallback" was false for four sessions -- a fallback run would have been blind to drift in five modules **including `MovementConfig`, the one that actually drifted.** Back-ported at B58. The header's hazard note runs in BOTH directions: a promotion updates BOTH copies, in the same session.

**Open threads.** Only CURRENCY drops print a `[DATA] Drop:` line -- the Item branch calls `AddItem` silently, so doubled `BannerTicket`/`TraitRerollToken` are invisible in the console; a print there is a cheap follow-up and is how B45's mis-routed faucet came to "look wired". Best-of-N picks the best of N NEW candidates and never compares against the unit's current values, so a 100%-Luck reroll can still DOWNGRADE (seen live: `DMG D->B RNG SS->C SPA A->D`) -- accepted by the user, but it is the complaint to expect. `StartingLives` Act 1 = 3 now has evidence: eight level-20 towers cleared 12/15 waves for 66,850 damage and still lost; holding 15/15 needed 24 towers at max meta/ascension/rolls. `STATE.md` (122) and both `CONTEXT.md` (153/154) are over their ADR-0006 caps and were ALREADY over before this session -- they need a prune pass.

**Harnesses (Game, both ship OFF).** `AutoPlaceForEndScreenTest`'s positions were ~260 studs off the path -- the documented cause of A7's "0 damage", uncorrected in `KnownIssues` since; fixed to the real `Path_Main` line. `MatchLifecycleSmokeTest` gained `DevDifficultyMode` (the `DevMatchModifiers` shape) so the Insane branch's GUARANTEED grants are reachable without a Lobby launch.

No schema change (v6). Lobby Remotes unchanged at 47. **Both Places must be REPUBLISHED by the user.**
...
**Three bugs found by verifying, not by reading.** An ENDED banner was showing a featured-rotation countdown. `DevPopup` resets itself to `""`, which re-fires its own changed signal, and the re-entrant call closed the popup the first call had just opened -- **a re-entry flag did NOT fix it**, because Roblox attribute signals are DEFERRED and the re-entrant call runs after the flag has been cleared; the fix is that `DevPopup` no longer self-resets and is a STATE rather than a pulse. And the screen read its server state once at open, so **a Luck buff bought while the screen was open never appeared and Show Chances kept quoting the unbuffed odds** -- it now re-polls every 5s while open, which is the only way a purchase landing in `ProcessReceipt` (possibly on another server, with no push remote by design) can reach the screen.
...
**USER: create four Developer Products and paste their ids into `GemPackConfig`.** A Developer Product cannot be created from a script. Checked at the user's request: the Experience has exactly ONE, `3711220080` "Premium Battlepass Season 1" (799 R$), which belongs to the battlepass -- pointing a gem pack at it would charge a player for the battlepass and hand them Gold, and would collide in `ReceiptService` the day the battlepass registers its own products. So the four ids stay `0` and are now marked as PLACEHOLDER in the config, per pack and in a banner comment, plus a loud boot warning while any pack is unconfigured. Each pack renders with a "Coming soon" button and `BuyGemPack` refuses `pack_not_configured` -- it never prompts with a zero id, which fails in front of the player with no explanation.
